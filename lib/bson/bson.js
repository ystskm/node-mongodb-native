...
var object2string = function(d) {
  var cnam = String( (d && d.constructor || { }).name );
  try {
    return Object.prototype.toString.call(d);
  } catch(e) {
    return '[' + (typeof d) + ' ' + cnam + ']';
  }
};
...
var isDate = function isDate(d) {
  try {
    return typeof d === 'object' && object2string(d) === '[object Date]';
  } catch(e) {
    return false;
  }
}
...
if(typeof Buffer.from != 'function' || typeof Buffer.alloc != 'function') {
  Buffer.from = (x, enc)=>Buffer(x, enc), Buffer.alloc = (x, ini, enc)=>Buffer(x).fill(ini == null ? 0: ini, enc);
}
 ...
 * @api public
 */
/* (2020.05.08 sakamoto) Has some bug
var circularObjects = null;
 e.g.) [RangeError [ERR_OUT_OF_RANGE] [ERR_OUT_OF_RANGE]: The value of "offset" is out of range. It must be >= 0 && <= 2365. Received 2375
  packElement (/Users/ystk_skm/git/node-mongodb-native-1.4.x/node_modules/bson/lib/bson/bson.js:909:60)
BSON.calculateObjectSize = function calculateObjectSize(object, serializeFunctions) {
  
  var top;
  if(circularObjects == null) {
    top = circularObjects = [ ];
  } else if(circularObjects.indexOf(object) != -1) {
    return 0;
  }
  
  var totalLength = (4 + 1);
  if(object && typeof object == 'object') { 
    circularObjects.push(object);
  }
  ...
  if(top === circularObjects) {
    top = circularObjects = null;
  }
  return totalLength;
    ...
    case 'object':
      if(value === null || value instanceof MinKey || value instanceof MaxKey
        ...
        if(value.scope != null && Object.keys(value.scope).length > 0) {
          ...
          var scopeObjectBuffer = supportsBuffer ? Buffer.alloc(scopeSize) : new Uint8Array(new ArrayBuffer(scopeSize));
    ...
    case 'function':
      ...
      if(value instanceof RegExp || Object.prototype.toString.call(value) === '[object RegExp]' || String.call(value) == '[object RegExp]') {
        ...
        if(serializeFunctions && value.scope != null && Object.keys(value.scope).length > 0) {
          ...
          buffer[index++] = 0;
          // Serialize the scope object
          var scopeObjectBuffer = Buffer.alloc(scopeSize);
          // Execute the serialization into a seperate buffer
          serializeObject(value.scope, checkKeys, scopeObjectBuffer, 0, serializeFunctions);
