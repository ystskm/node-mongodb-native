...
Db.prototype.open = function(callback) {
  ...
  //10-28-2013 [ystskm] add for impetuosity request >>
  ...
  if(self.serverConfig instanceof Server || self.serverConfig instanceof ReplSet || self.serverConfig instanceof Mongos) {
    ...
    if(self.serverConfig._serverState === 'connecting') {
      self.connectionPool.isConnected() ? connectCallback()
        : self.connectionPool.once('poolReady', connectCallback);
      return;
    }
    // << 

    // Attempt to connect
    self.serverConfig.connect(self, connect_options, connectCallback);
    ...
    function connectCallback(err, result) {
      ...
      // 10-28-2013 [ystskm] modify/add for impetuosity request >>
      var arr = self.wait;
