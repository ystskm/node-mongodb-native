...
function Db(databaseName, serverConfig, options) {
  ...
  // 06-22-2015 [ystskm] modify for writeConcern only >>
  //if(this.options.safe == null
  if(this.options.writeConcern == null
    && this.options.safe == null
  // <<
    && this.options.w == null
    && this.options.j == null
...
Db.prototype.open = function(callback) {
  ...
  var db = this, self = db;
  var server = db.serverConfig, cp = server.connectionPool;
  
  // 10-28-2013 [ystskm] add for impetuous request -->
  if(db._state == 'connected') {
    return callback(null, db);
  }

  if(isArray(db.wait)) {
    db.wait.push(function() {
      callback(null, db);
    });
    return;
  }
  ...
  // 10-04-2017 [ystskm] NO NEED TO CHECK! (Accept duplicated call by "this.wait") -->
...
//  }
  ...
  if(db.readPreference != null) {
    server.setReadPreference(db.readPreference);
  }
  ...
  db._state = 'connecting';

  // Set up connections
  if(server instanceof Server || server instanceof ReplSet || server instanceof Mongos) {

    // Ensure we have the original options passed in for the server config
    var connect_options = {};
    for(var name in server.options) {
      connect_options[name] = server.options[name]
    }
    connect_options.firstCall = true;

    // 1-8-2014 [ystskm] avoid "Cannot determine state of server" 
    // when synchronously db.open() -->
    if(server._serverState === 'connecting') {
      cp.isConnected() ? connectCallback(): cp.once('poolReady', connectCallback);
      return;
    }
    // <--

    // Attempt to connect
    server.connect(db, connect_options, connectCallback);

    function connectCallback(err, result) {
      ...
      // 10-28-2013 [ystskm] modify/add for impetuous request -->
      var arr = db.wait;
      if(isArray(arr))
 ...
 * @api public
 */
//10-28-2013 [ystskm] modify to keep-alive server
...
var transformCollections = function(databaseName) {
  ...
  return function(results) {
    ...
    if(results == null) {
      ...
      var index = String(results.name).indexOf(matching);    
      if(index == 0) {
        results.name = String(results.name).substr(index + matching.length);
      }
    }
...
Db.prototype.collection = function(collectionName, options, callback) {
  ...
  db.listCollections({ name:collectionName }).toArray(function(err, collections) {
    ...
    try {
      return callback(NULL, new Collection(db, collectionName, db.pkFactory, options));
    } catch(err) {
      return callback(err, NULL);
    }
...
Db.prototype.command = function(selector, options, callback) {
  ...
  if(options.writeConcern) {
    selector.writeConcern = options.writeConcern;
  }

  // Check if we need to set slaveOk
  if(options.readPreference != 'primary') {
...
Db.prototype.executeDbCommand = function(command_hash, options, callback) {
  // (2018.07.19 sakamoto) change fit with executeDbCommandCursor
  var db = this;
  if(callback == null) { callback = options; options = {}; }
  var cmd = DbCommand.createDbSlaveOkCommand(db, command_hash, options);
  this._executeQueryCommand(cmd, options, function(err, result) {
 ...
 * 
 */
Db.prototype.executeDbCommandCursor = function(command_hash, options, callback) {
  // (2018.07.19 sakamoto) add for aggregate cursor command (v4.0)
  var db = this;
  if(callback == null) { callback = options; options = {}; }
  try {
    if(callback) callback(NULL, new CommandCursor(db, { collectionName: DbCommand.SYSTEM_COMMAND_COLLECTION }, command_hash, options));
  } catch(e) {
...
Db.prototype.createIndex = function(collectionName, fieldOrSpec, options, callback) {
  ...
  createIndexUsingCreateIndexes(self, collectionName, fieldOrSpec, options, function(err, result) {
    ...
    if(err.name == 'MongoError') { 
      // Detect util.toError-ed object
      // TODO retry by "createCreateIndexCommand" if it's not compatible error.
      return callback(err);
    }
...
Db.prototype._executeQueryCommand = function(db_command, options, callback) {
  ...
  // Specific connection
  var connection = options.connection || self.serverConfig.checkoutReader();
  // [OLD]
...
Db.prototype._executeInsertCommand = function(db_command, options, callback) {
  ...
  // Specific connection
  var connection = options.connection || self.serverConfig.checkoutWriter();
  // [OLD]
  // var connection = options.connection;
  // Check if the connection is actually live
  // if(connection
 ...
 * @ignore
 */
// [ 5. 8.2015] changed by ystk_skm for options.writeConcern.
// [20.12.2015] keep self.options.writeConcern parameter if not options are specified
var _getWriteConcern = function(self, options) {
  // Final options
  var finalOptions = self.options.writeConcern || {w:1};

  // Though logic if options are set.
  ...
  if(options.w != null || typeof options.j == 'boolean' || typeof options.journal == 'boolean' || typeof options.fsync == 'boolean') {
    ...
    // [20.12.2015] over-writable "safe" option
    ...
    // [20.12.2015] the first priority is "finalOptions"
    finalOptions = _setWriteConcernHash({
      w: finalOptions.w == null ? self.safe.w: finalOptions.w,
      journal: finalOptions.journal == null ? self.safe.journal: finalOptions.journal,
      j: finalOptions.j == null ? self.safe.j: finalOptions.j,
      fsync: finalOptions.fsync == null ? self.safe.fsync: finalOptions.fsync,
      wtimeout: finalOptions.wtimeout == null ? self.safe.wtimeout: finalOptions.wtimeout
    });
  } else if(typeof self.safe == "boolean") {
    // [20.12.2015] the first priority is "finalOptions"
    finalOptions = { 
      w: finalOptions.w == null ? (self.safe ? 1 : 0): finalOptions.w 
    };
  }
  }
