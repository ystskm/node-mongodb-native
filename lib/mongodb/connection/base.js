...
var CallbackStore = function() {
  ...
  EventEmitter.call(cbs);
  // Add a info about call variable
  cbs._notReplied = {};
  cbs.id = (id += 1) & 0xffffff;
}
...
Base.prototype._registerHandler = function(db_command, raw, connection, exhaust, callback) {
  ...
  this._callBackStore._notReplied[db_command.getRequestId().toString()] = {
    start: new Date().getTime(), 'raw': raw, connection:connection, exhaust:exhaust
  };
}
 ...
 * @api private
 */
Base.prototype._flushAllCallHandlers = function(err, doc) {

  var keys = Object.keys(this._callBackStore._notReplied);
  for(var i = 0; i < keys.length; i++) {
    this._callHandler(keys[i], doc, err);
  }
...
Base.prototype._callHandler = function(id, document, err) {
  ...
  if(this._callBackStore.listeners(id).length >= 1) {
    ...
    if(callback.listener) {
      // Old node (may <= v6.11.0) module have "listener" key for each listener
    ...
    try {
      ...
      if(err) {
        throw utils.toError(err);
      }
      // Execute the callback if one was provided
      if(typeof callback == 'function') callback(null, document, info.connection); 

    } catch(e) {
      var er = utils.toError(e);
      self._emitAcrossAllDbInstances(self, null, "error", er, self, true, true);
