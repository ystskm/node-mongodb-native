...
if(debug
  && (debug == 'true' || (new RegExp(DEBUG_CATEGORY + '|' + DEBUG_KEY)
      .test(debug)))) {
  debug = function(s) {
    util.error(FILE_NAME + ': ' + (typeof s == 'function' ? s(): s));
  };
} else {
  debug = function() {
  };
    ...
    */
    ...
    _self.pools.forEach(function(pool, rank) {
      ...
      function onRankPoolReady() {
        util.log('[connection_pool.js] pool ready for ' + _self.host + ':'
          + _self.port + ' (rank:' + pool.rank + ', size:' + pool.size + ')');
        if(--len == 0)
          _self.emit('poolReady');
      }
      function onRankPoolReconnect(n) {
        util.log('[connection_pool.js] pool reconnect for ' + _self.host
          + ':' + _self.port + ' (rank:' + pool.rank + ', size:' + pool.size
          + ', times:' + n + ')');
        if(--len == 0)
          _self.emit('poolReconnect');
      }
    });
    // <<
    /*
