  ...
  function onPoolMessage(message, connection) {
    ...
    try {
      ...
      mongoReply.parseBody(message, connectionPool.bson, callbackInfo.info.raw, function(err) {
        ...
        var fatal = err || document.ok === 0 && document.code;
        ...
        // exception: DBClientBase::findN: transport error: 10.0.101.187:27017 ns: 
        // code: 10276
        ...
        if(fatal) {
          ...
          console.log('  at pool:#' + connectionPool.pid + ', connection:#' + connection.id, ', reply:', document);
          server.__executeAllCallbacksWithError(err || document);
          if(cp.isConnected()) cp.restart();
        }
