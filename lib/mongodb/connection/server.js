...
Server.prototype.connect = function(dbInstance, options, callback) {
  ...
  //6-4-2013 [ystskm] not to reconnect >>
  if(server.connectionPool)
    return server.connectionPool.isConnected() ? alreadyConnected()
      : server.connectionPool.once('poolReady', alreadyConnected);
  function alreadyConnected() {
    var internalCallback = callback;
    callback = null, internalCallback(null, dbInstance, server);
  }
  // <<
  ...
  var connectCallback = function(_server) {
    return function(err, reply) {  
      ...
      // >> 07-03-2015 ystskm >>
      // for ranked pool
      _server.connectionPool.serverCapabilities = server.serverCapabilities;
      // << 
  ...
  connectionPool.on("message", function(message, connection) {
    ...
    try {
      ...
      if(mongoReply.messageLength != message.length) {
        ...
        if(callbackInfo == null || callbackInfo.info == null) {
          console.log('[Mongodb](connection/server.js) Abort if not a valid callbackInfo, don\'t try to call it')
          return;
        }
        ...
        mongoReply.parseBody(message, connectionPool.bson, callbackInfo.info.raw, function(err) {
          ...
          if(fatal) {
            console.log(new Date().toGMTString() + ' - [Mongodb] Detect ' + fatal);
            console.log('  at connection:#' + connection.id, ', reply:', document);
          }
  ...
  // error
  connectionPool.on("parseError", function(err, requestId) {
