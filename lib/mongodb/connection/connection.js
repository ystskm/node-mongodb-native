...
var Connection = exports.Connection = function(id, socketOptions) {
  ...
  // console.log('CHECK - maxBsonSize:' + this.maxBsonSize, ', maxMessageSizeBytes:' + this.maxMessageSizeBytes, ', maxNumberOfDocsInBatch:' + this.maxNumberOfDocsInBatch);
...
//[10-31-2013 ystskm] add for ranked connection
...
Connection.prototype.write = function(command, callback) {
  try {
    ...
    if(Array.isArray(command)) {
      for(var i = 0; i < command.length; i++) {
        try {
          ...
          // [03-26-2014 ystskm] not return when callback is not defined >>
          throw err;
    ...
    // [10-31-2013 ystskm] add for detecting unexpected no return >>
    //if(typeof callback === 'function') callback(err);
    console.log(new Date().toGMTString() + ' - [mongodb connection.js::write()] Write error.');
    try {
      console.log(err);
    } catch(e) {
      console.log(new Date().toGMTString() + ' - [mongodb.connection.js] console occurs error => ', err = err && err.message || 'unknown write error');
    }
    if(typeof callback === 'function') callback(err);
    else throw err;
    // <<
...
Connection.prototype.close = function() {
  ...
  // => Connection object MUST emit "close" event manually.
...
Connection.CONNECTION_WAITING_TIMEOUT = 30000;
// <--
...
if(Debug) {
  if(Debug == 'true' || new RegExp(DEBUG_CATEGORY + '|' + DEBUG_KEY).test(Debug)) {
    // running as debug mode
    Debug = TRUE;
  } else {
    Debug = FALSE;
    debug = Function();
