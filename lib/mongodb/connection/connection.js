...
Connection.prototype.start = function() {
  ...
  if(this.socketOptions.ssl) {
    ...
    // console.log("---------------------------------------------------")
    // console.dir(tls_options)

    // Attempt to establish a TLS connection to the server
    var cleartext = tls.connect(this.socketOptions.port, this.socketOptions.host, tls_options, function() {
      // If we have a ssl certificate validation error return an error
      if(cleartext.authorizationError && validate_certificates) 
        return self.emit("error", new Error(cleartext.authorizationError), self);
