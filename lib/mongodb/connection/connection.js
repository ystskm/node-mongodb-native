var utils = require('./connection_utils'), inherits = require('util').inherits, net = require('net'), EventEmitter = require('events').EventEmitter, inherits = require('util').inherits, binaryutils = require('../utils'), tls = require('tls');
...
var Connection = exports.Connection = function(id, socketOptions) {
  ...
  this.maxBsonSize = socketOptions.maxBsonSize ? socketOptions.maxBsonSize
    : Connection.DEFAULT_MAX_BSON_SIZE;
  // Bson object
...
//4-4-2013 [ystskm] move to bottom >>
// Set max bson size
...
Connection.prototype.start = function() {
  ...
  if(this.socketOptions.ssl) {
    ...
    this.connection.setTimeout(this.socketOptions.connectTimeoutMS != null
      ? this.socketOptions.connectTimeoutMS: this.socketOptions.timeout);
    ...
    this.connection = net.createConnection(this.socketOptions.port,
      this.socketOptions.host);
...
Connection.prototype.isConnected = function() {
  return this.connected && !this.connection.destroyed
    && this.connection.writable && this.connection.readable;
}

// [ystskm]
Connection.prototype.isUsed = function() {
  return this.store != null;
}

// [ystskm]
Connection.prototype.isFree = function() {
  return this.isConnected() && !this.isUsed();
}
  ...
  try {
    ...
    if(Array.isArray(command)) {
      for( var i = 0; i < command.length; i++) {
        ...
        if(!this.socketOptions['disableDriverBSONSizeCheck']
          && binaryCommand.length > this.maxBsonSize)
          return callback(new Error(
            "Document exceeds maximal allowed bson size of " + this.maxBsonSize
              + " bytes"));
    ...
    util.error('[mongodb connection.js::write()] Write error.');
...
Connection.prototype.close = function() {
  ...
  // removeListeners TODO resetHandler is not affect? Node's bug?
  delete this._events, delete this.connection._events;
}
...
var connectHandler = function(self) {
  return function() {
    ...
    self.connection.setTimeout(self.socketOptions.socketTimeoutMS != null
      ? self.socketOptions.socketTimeoutMS: self.socketOptions.timeout);
      ...
      if(self.bytesRead > 0 && self.sizeOfMessage > 0) {
        ...
        if(remainingBytesToRead > data.length) {
          ...
          try {
            ...
            if(self.logger != null && self.logger.doError)
              self.logger.error("parseError", errorObject);
        ...
        if(self.stubBuffer != null && self.stubBuffer.length > 0) {
          ...
          if(data.length > 4) {
            ...
            if(sizeOfMessage > 4 && sizeOfMessage < self.maxBsonSize
...
var endHandler = function(self) {
  return function() {
    ...
    // Emit end event
    self.emit("end", {
      err: 'connection received Fin packet from [' + self.socketOptions.host
        + ':' + self.socketOptions.port + ']'
    }, self);
  }
}
...
var timeoutHandler = function(self) {
  return function() {
    self.emit("timeout", {
      err: 'connection to [' + self.socketOptions.host + ':'
        + self.socketOptions.port + '] timed out'
    }, self);
  }
}
...
var errorHandler = function(self) {
  return function(err) {
    ...
    // Emit error
    self.emit("error", {
      err: 'failed to connect to [' + self.socketOptions.host + ':'
        + self.socketOptions.port + ']'
    }, self);
  }
}
...
var closeHandler = function(self) {
  return function(hadError) {
    ...
    if(hadError && !self.connected) {
      ...
      // Emit close
      self.emit("close", {
        err: 'connection closed to [' + self.socketOptions.host + ':'
          + self.socketOptions.port + ']'
      }, self);
    }
  }
...
Connection.CONNECTION_WAITING_TIMEOUT = 3000;
//<<
