...
//4-4-2013 [ystskm] add >>// Set processor, setImmediate if 0.10 otherwise nextTick
//debug function set
var util = require('util'), debug = process.env.NODE_DEBUG;
var DEBUG_CATEGORY = 'mongodb-native', DEBUG_KEY = 'db', FILE_NAME = __filenam.split('/').slice(-1).toString();
...
Server.prototype.connect = function(dbInstance, options, callback) {
  ...
  //4-4-2013 [ystskm] add >>
  if(this._serverState == 'connected') {
    if(this.dbInstances.indexOf(dbInstance) == -1)
      this.dbInstances.push(dbInstance);
  ...
  //4-4-2013 [ystskm] remove >>
  //It worse for continuous service.
  ...
  connectionPool.on("poolReady", connectionPoolReady);
  ...
  connectionPool.on("message", function(message) {
    ...
    try {
      ...
      if(mongoReply.messageLength != message.length) {
        ...
        var requestId  = mongoReply.responseTo.toString();
        callbackInfo = Server.Db.findCallbackInfo(requestId);
        dbInstanceObject = callbackInfo.db;
        /*
        ...
        */
        ...
        if(mongoReply.requestId > 0 && mongoReply.cursorId.toString() != "0" 
          && callbackInfo.info && callbackInfo.info.exhaust) {
 ...
 * @ignore
 */
Server.prototype.checkoutReader = function(rank) {
  // Check if are allowed to do a checkout (if we try to use an arbiter f.ex)
