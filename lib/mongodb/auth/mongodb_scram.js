  ...
  while(connections.length > 0) {    
    ...
    var executeScram = function(connection) {
      ...
      db.db(authdb).command(cmd, { connection: connection }, function(err, r) {
        ...
        var clientKey = Buffer.from(hmac.digest('base64'), 'base64');

        // Create the stored key
        var hash = crypto.createHash('sha1');
        hash.update(clientKey);
        var storedKey = Buffer.from(hash.digest('base64'), 'base64');
        ...
        var clientSig = Buffer.from(hmac.digest('base64'), 'base64');

        // Create client proof
        var clientProof = f("p=%s", Buffer.from(xor(clientKey, clientSig)).toString('base64'));
        ...
        var serverKey = Buffer.from(hmac.digest('base64'), 'base64');

        // Generate server signature
        var hmac = crypto.createHmac('sha1', serverKey);
        hmac.update(Buffer.from(authMsg))
        var serverSig = Buffer.from(hmac.digest('base64'), 'base64');
