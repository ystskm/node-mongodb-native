...
if(debug && (debug == 'true' || (new RegExp(DEBUG_CATEGORY + '|' + DEBUG_KEY).test(debug)))) {
  debug = function(s) { util.error(FILE_NAME + ': ' + s);};
} else {
  debug = function() {};
}
...
// 4-13-2013 [ystskm] add
// TODO check chained status when error, see db.js _fireCallbackErrors.
CallbackStore.prototype.registerHandler = function(db_command, options, callback) {
  
  var self = this;
  
  var start = new Date().getTime(), chainedIds = [];
  options.db = this._db, options.start = start;
  [].concat(db_command).forEach(forEachCommand);
  
  if(chainedIds.length > 1)
    options.chained = chainedIds;
  
  function forEachCommand(cmd, i){
    // accept raw requestId for reRegister
    var requestId = typeof cmd == 'string' ? cmd: cmd.getRequestId().toString();
    // push information to memory
    chainedIds.push(requestId), self._notReplied.push(requestId);
    NotReplied[requestId] = options;
    // add handler (used via ._callHandler())
    self.once(requestId, function(err, document) {
      callback.call(null, err, document), self.removeHandler(requestId);
    });
  }
  
}

CallbackStore.prototype.removeHandler = function(requestId) {
  
  this.removeAllListeners(requestId);
  
  var opt = NotReplied[requestId], ids = this._notReplied;
  var chain = opt.chained || [];
  
  var nr_idx = ids.indexOf(requestId), ch_idx = chain.indexOf(requestId);
  if(nr_idx != -1)
    ids.splice(nr_idx, 1);
  if(ch_idx != -1)
    chain.splice(ch_idx, 1);
  
  delete NotReplied[requestId];
  
}

CallbackStore.prototype.removeAllHandlers = function() {
  // TODO test and check need ?
  var self = this;
  this._notReplied.forEach(function(id){
    self.removeHandler(id);
  });
}

CallbackStore.prototype.responseErrorAll = function(err){
  // TODO test repl_set.js
  var self = this;
  this.each(function(id){
    self.emit(id, err, null);
  });
}

CallbackStore.prototype.each = function(fn, branch){
  if(typeof branch != 'function')
    branch = function(){
      return true;
    };
  this._notReplied.forEach(function(id) {
    if(branch(NotReplied[id], opt))
      fn(id);
  });
}

CallbackStore.prototype.hasNotReplied = function(){
  return !!this._notReplied.length;
}

CallbackStore.prototype.findOptions = function(id) {
  return this._notReplied[id];
}

CallbackStore.prototype.findHandler = function(id){
  return this.listeners(id).length > 0 ? this.listeners(id)[0] : null;
}
...
var reaper = function(dbInstance, reaperInterval, reaperTimeout) {
  ...
  if((currentTime - dbInstance._lastReaperTimestamp) >= reaperInterval) {
    ...
    var cbs = dbInstance._callBackStore;
    cbs.each(function(id){
      cbs.emit(id, new Error("operation timed out"), null);
    }, function(id, info) {
      return (currentTime - info.start) > reaperTimeout;
    });
...
Db.prototype.open = function(callback) {
  ...
  if(Array.isArray(this.wait))
    return this.wait.push(function() {
      callback(null, self);
    });
  ...
  if(self.serverConfig instanceof Server || self.serverConfig instanceof ReplSet 
      || self.serverConfig instanceof Mongos) {
 ...
 * @api public
 */
// 4-12-2013 [ystskm] modify to alive server
Db.prototype.close = function(forceClose, callback) {
  ...
  if(typeof forceClose == 'function') {
    callback = forceClose, forceClose = false;
  } else if(typeof forceClose == 'boolean') {
    this._applicationClosed = forceClose;
  }

  // db.close usually not stop server.
  // otherwise, connection pool will be cleared.
  
  // Remove all listeners and close the connection
  // or remain server.
  forceClose ? this.serverConfig.close(closeAllCallback): sliceInstance();
  
  function closeAllCallback(err, result) {
  ...
  function sliceInstance() {
    
    // slice dbInstance from instances in server
    var allServerInstances = self.serverConfig.allServerInstances();
    if(!Array.isArray(allServerInstances))
      return closeAction();
    
    allServerInstances.forEach(function(server) {
      if(Array.isArray(server.dbInstances) && server.dbInstances.length > 1) {
        var idx = null;
        while((idx = server.dbInstances.indexOf(self)) != -1)
          server.dbInstances.splice(idx, 1);
      }
    });
  ...
  function closeAction(err, result){
...
Db.prototype._registerHandler = function(db_command, raw, connection, exhaust, callback) {
  ...
  this._callBackStore.registerHandler(db_command, {
    raw: raw,
...
Db.prototype._reRegisterHandler = function(newId, object, callback) {
  this._callBackStore.registerHandler(newId, object.info, object.callback.listener);
  /*
  // Add the callback to the list of handlers
...
Db.prototype._callHandler = function(id, document, err) {
  this._callBackStore.emit(id, err, document);
  /*
...
Db.prototype._hasHandler = function(id) {
  return this._callBackStore.hasNotReplied();
  /*
  // If there is a callback peform it
...
Db.prototype._removeHandler = function(id) {
  this._callBackStore.removeHandler(id);
  /*
...
Db.prototype._findHandler = function(id) {
  return {callback: this._callBackStore.findHandler(id)};
  /*
  var info = this._callBackStore._notReplied[id];
  // Return the callback
...
Db.findCallbackInfo = function(id) {
  var opts = NotReplied[id], ret = opts.db._findHandler(id);
  return ret.db = opts.db, ret.info = opts, ret;
}
...
var __executeQueryCommand = function(self, db_command, options, callback) {
  ...
  if(typeof callback === 'function' && !onAll) {
    ...
    //4-4-2013 [ystskm] modify >>
    //for rank service
    //connection.write(db_command, function(err) {
    connection.write(db_command, options, function(err) {
    // <<
      if(err != null) {
