...
Commands.prototype.open = function(callback) {
  ...
  function wpCountcb(err, cnt) {
    ...
    gc.getStatsData(function(err, stats) {
      ...
      OutDebug(0, 'waitProcess', function() {
        util.log('wpCountcb: ' + gsid + '; MongoObject._id: '
      ...
      OutDebug(0, 'waitProcess', function() {
        util.log('Go to recover. (after ' + timeout + ' ms)');
      ...
      function setWaitingTimeout() {
        ...
        var timeout = Commands.DEFAULT_SAVE_RETRY_BASE_MILLISECOND
          + Math.random() * Commands.DEFAULT_SAVE_RETRY_MILLISECOND;

        // execute next waitProcess after "base" + "random" millisecond
        // for distributed environment.
        // note that "base" should be bigger than mongodb write concern time.
...
Commands.prototype.recoverCondition = function(now_stat, try_sel, callback,
  ...
  var now = Date.now(), recoverNow = function(compare) {
    OutDebug(0, 'recoverCondition', function() {
      util.log('recoverNow (phase=' + phase + ', last update before '
        + (now - compare) + ' ms ): '), console.log(gc.filename);
    });
...
Commands.prototype.recover = function(ns_write_id, callback) {
  ...
  // real retry log
  OutDebug(0, 'recover', function() {
    util.log('Recover file. (ns_write_id = ' + ns_write_id + ')');
  ...
  function execRandomTimeRetryAfterRecover(err) {
    ...
    OutDebug(0, 'recover', 'retry after ' + timeout + ' ms.');
...
Commands.prototype.eventLock = function(key, lockId, statOverwrite, callback) {
  ...
  function setStatus(err, stat) {
    ...
    if(statOverwrite === false && !gc.in_queue) {
      ...
      gs.statEnd(), gs.waitForWrite(function() {
        ...
        gc.in_queue = 'write', timer.setTimeout(function() {
          gs.statBegin(getStatus);
    ...
    // others are set in_queue "write"
    gc.in_queue = gc.in_queue || true;
  ...
  function upsetStat(stat) {
    ...
    // "val" is the setting object for this process
    val = {
      update: now,
      _writing: StatIs.LOCKED
    };
    ...
    // Cleanup lock trash, both "READ" and "WRITE"
    // (e.g. kill when processing)
    ['READ', 'WRITE'].forEach(function(ok) { // operation key

      var cleanup = function(lk) { // lock key
        // if still treat as "operating thread"
        // "_l.time" == now.getTime().
        var limit = Commands.STAT_FORCE_UPDATE_LIMIT_MILLISECOND;
        if(_l.time - stat[ok][lk] < limit)
          return;
        // else, delete
        delete stat[ok][lk];
      };
      ...
      // both, "READ" and "WRITE"
      Object.keys(stat[ok]).forEach(cleanup);

    });

    // DON'T start reading before write process exist.
    if(key == Action.READ && Object.keys(now_stat[Action.WRITE]).length) {

      // set waiting timer
      // TODO need max timeout or max repeat time?
      timer.setTimeout(function() {
        gs.statBegin(getStatus);
      }, Commands.DEFAULT_READ_RETRY_MILLISECOND);

      // release per-file-status lock
      gs.statEnd();
      return;
  ...
  function upsetcb(err, n) {
    ...
    gc.existStatsData(sel, function(err, stat) {
      // TODO handle error?
      res = stat, (stat ? locked: progressCheckAndRetry)(sel);
    });
  ...
  function progressCheckAndRetry(sel) {
    ...
    try_sel = gc.recoverCondition(now_stat, try_sel, getStatus, 'L0') === true
      ? sel: null;
  }
  ...
  function locked() {
    ...
    OutDebug(3, 'eventLock', function() {
      console.log('eventLock succeeded.', gc.filename, gc.gs.gsid);
    });
  ...
  function release(r_callback) {
    gc.upsetStatsData({
      ...
      OutDebug(3, 'eventLock', function() {
        console.log('eventLock released.', gc.filename, gc.gs.gsid);
      });
...
Commands.prototype.eventUnlock = function(callback) {
  ...
  function progressCheckAndRetry(sel) {
    ...
    try_sel = gc.recoverCondition(now_stat, try_sel, getStatus, 'L1') === true
      ? sel: null;
  }
  ...
  function statDequeue() { // for eventUnlock()
    ...
    if(gc.in_queue == 'write') // when in_queue == 'write', go next write
 ...
 * @constant
 */
Commands.DEFAULT_SAVE_RETRY_BASE_MILLISECOND = 800;
Commands.DEFAULT_SAVE_RETRY_MILLISECOND = 3000;

/**
 * Read retry interval when writing.
 * 
 * @constant
 */
Commands.DEFAULT_READ_RETRY_MILLISECOND = 300;
