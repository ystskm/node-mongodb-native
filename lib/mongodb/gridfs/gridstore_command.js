...
_reader.prototype.end = function(endCallback) {
  ...
  function _nth(n) {
    ...
    function nthcb(err, chunk_origin) {
      ...
      if(self._fin != null) // if stop signal recieved
        fin = true, typeof self._fin === 'number' && (read = self._fin);
  ...
  function emitDataBuffer(n, chunk) {
    self.emit(DataEvent.Data, n, chunk);
  }

  function emitDataString(n, chunk) {
    self.emit(DataEvent.Data, n, chunk.toString(self.encode));
  }

  function error(err) {
    self.emit(DataEvent.Error, err);
    self.removeAllListeners();
  }

  function finish() {
    self.emit(DataEvent.End, read);
    self.removeAllListeners();
  }
};
...
/**
 * Check the condition and kick recover logic. if only wait is needed, pass
 * "false" to recover logic.
 */
...
Commands.prototype.recover = function(ns_write_id, callback) {
  ...
  // real retry log
  // TODO remove
  OutMessage.log('recover', 'Recover file datas of: ' + self.gs.gsid + "; "
...
Commands.prototype.eventLock = function(key, lockId, statOverwrite, callback) {
  ...
  function upsetcb(err, n) {
    ...
    function progressCheckAndRetry() {
      ...
      try_sel = self.recoverCondition(now_stat, try_sel, getStatus) === true
        ? sel: null;
    }
...
Commands.prototype.eventUnlock = function(callback) {
  ...
  function upsetcb(err, n) {
    ...
    self.existStatsData(sel, function(err, data) {
      // TODO handle error?
      res = data, (data ? lockEnd: progressCheckAndRetry)(sel);
    });
