...
var fs = require('fs'), util = require('util'), os = require('os');
var Emitter = require('events').EventEmitter, Chunk = require('./chunk').Chunk;
var timer = require('../timer');

var PREVIOUS_HANDLED;
if(!global.Promise) global.Promise = require('es6-promise');
...
Commands.prototype.errHandler = function(msg, err, handle) {
  ...
  if(err.message == Err.ENOENT) {
    noent_er = new Error(Err.ENOENT + os.EOL + [timemsg, filemsg].join(', '));
    noent_er.stack = err.stack || NULL;
    handle(noent_er);
    return;
  }
  ...
  if(PREVIOUS_HANDLED === gc) {
    ...
    timemsg += ' - Recursive error message?';
    console.log(timemsg, msg, err, stack, handle);
    return;
...
Commands.prototype.open = function(callback) {
  ...
  function getLatest() {
    if(!gc.filename) return openCallback(new Error(Err.EBADF));
  ...
  function registerPresent(er, obj) {
    ...
    if(er) return openCallback(er);

    if(obj == null && OpenModeIs.read(mode)) return openCallback(new Error(
      Err.ENOENT));
...
_reader.prototype.end = function(endCallback) {
  ...
  function countcb(er, cnt) {
    ...
    if(!cnt) return error(new Error('No chunk data for ' + gc.filename
  ...
  function _nth(n) {
    ...
    function nthcb(err, chunk_origin) {
      ...
      if(offsetRemains > 0) {
        if(offsetRemains >= chunk_len) return offsetRemains -= chunk_len, callNext();
...
Commands.prototype.write = function(data, options, callback) {
  ...
  function runWrite() {
    ...
    gc
      .writeBuffer(Buffer.isBuffer(data) ? data: new Buffer(data), EOF, finalClose, writeCallback);
  }
...
Commands.prototype.writeBuffer = function(buffer, EOF, finalClose, callback) {
  ...
  if(isFunction(EOF)) callback = EOF, finalClose = EOF = false
  else if(isFunction(finalClose)) callback = finalClose, finalClose = false;
  ...
  function writeProcessor(rsl, rej) {
    ...
    function convertToChunks(leftOverData) {
      ...
      if(!gc.currentChunk) return rej(new Error(gc.filename + " not opened."));

      if(OpenModeIs.read(gc.mode)) return rej(new Error(gc.filename
        + " not opened for writing"));

      if(leftOverData) buffer = leftOverData;
...
Commands.prototype.buildMongoObject = function(writing, callback) {
  ...
  if(isFunction(writing)) callback = writing, writing = FileIs.STABLE;
  ...
  if((gc.currentChunk || '').chunkSize) mongoObject['chunkSize'] = gc.currentChunk.chunkSize;
  ...
  gc.db.command(md5Command, function(er, results) {
    ...
    if(er == null && results) mongoObject.md5 = results.md5;
...
Commands.prototype.close = function(callback) {
  ...
  var closingCallback = function(er) {
    ...
    try {
      if(er) throw er;
      gc.rewind(), gs.close(closeCallback);
    } catch(e) {
      gc.errHandler(e, callback);
    }
  };
  ...
  if(!gc._no) return closingCallback();

  if(OpenModeIs.read(mode)) return closingCallback();

  // update files stats
  gc.filesCollection(function(er, col) {

    if(er) return closingCallback(er);
...
Commands.prototype.stat = function(meta, callback) {
  ...
  if(isFunction(meta)) callback = meta, meta = false;

  var statCallback = function(er, stat) {
    // try { ... } catch(e) { ... } to avoid the unexpected unlocked finish
    try {
      if(er) throw er;
      callback(null, stat, gc);
    } catch(e) {
      gc.errHandler(e, callback);
    }
  };
  ...
  if(gc._no == null) return statCallback(Err.EPERM);
...
Commands.prototype.deleteFiles = function(key, chunks_del, callback) {
  ...
  if(isFunction(key)) callback = key, chunks_del = false, key = gc.filesId;
  else if(isFunction(chunks_del)) callback = chunks_del, chunks_del = false;
...
Commands.prototype.deleteChunks = function(f_id, callback) {
  ...
  if(isFunction(f_id)) callback = f_id, f_id = gc.filesId;
...
for( var name in Collections) {
  (function(fname, space) {
    Commands.prototype[fname] = function(callback) {
      ...
      if(gc[k]) return callback(null, gc[k]);
...
Commands.prototype.chunkIn = function(options, callback) {
  ...
  if(OpenModeIs.read(gc.mode)) return gc
    .errHandler('ChunkIn requires mode "w" or "w+".', callback);
  ...
  // set callback caller
  if(isFunction(callback)) ee.on('error', last).on('end', last);
  ...
  function afterStat(er, stats) {
    ...
    if(er) return gc.errHandler('stats failed for ' + filepath, er, ee);

    // must be write for update/copy files data.
    if(!(stats || '').size) contents = contents || '';
...
var OutMessage = function(type, fn, message) {
  ...
  if(arguments.length == 2) message = fn, fn = type;

  message = OutMessage.message(fn, message);
  switch(type) {

  case 'error':
    if(Array.isArray(message)) console.error.apply(console, message);
    else
      console.error(message);
    return;

  default:
    if(Array.isArray(message)) console.log.apply(console, message);
    else
      util.log(message);
    return;
...
OutMessage.message = function(fn, mess) {
  ...
  if(Array.isArray(mess)) mess.unshift(prefix)
  else
    mess = prefix + mess;
...
function errFnGen(_mes, _mtd) {
  ...
  return function(mes, mtd) {
    if(_mes && mes) mes = _mes + mes;
    if(_mtd && mtd) mtd = mtd + _mtd;
...
function errGen(mes, method) {
  mes = mes ? mes + ' ': ''; // "Error: " is given by console function
  mes += (method ? 'at ' + method + '()': '') + ' in gridCommands.';
  return mes;
}
