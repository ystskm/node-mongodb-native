...
_reader.prototype.end = function(endCallback) {
  ...
  function onEnd(byteRead) {
    endCallback.call(gc, null, byteRead, reader.encode == null ? Buffer
  ...
  function error(err) {
    careLeak(), reader.emit(DataEvent.Error, err);
  }

  function finish() {
    careLeak(), reader.emit(DataEvent.End, read);
  }
