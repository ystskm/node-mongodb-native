...
var OutDebug = process.env.NODE_DEBUG ? function(fn, message) {
  OutMessage('debug', fn, message);
}: Function();
...
Commands.prototype.eventUnlock = function(callback) {
  ...
  function statDequeue() { // for eventUnlock()
    ...
    if(is_mine[Action.WRITE]) // in_queue == 'write' and mine.
