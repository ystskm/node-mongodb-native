...
Commands.prototype.errHandler = function(msg, err, handle) {
  ...
  var filemsg = gc.filename ? '(filename: ' + gc.filename + ')'
    : '(unknown filename)';
  msg = (msg ? msg + ":\n": '') + filemsg;

  // output error message.
  // TODO ignore "ENOENT" and so on.
  util.log(msg), console.error(err);

  if(err)
    err.message = [filemsg, err.message].join("\n");
  else
    err = new Error(filemsg);
...
Commands.prototype.open = function(callback) {
  ...
  var openCallback = function(er) {
    er ? gc.errHandler(er, callback): callback(null, gc);
  };
  ...
  function getLatest() {
    gc.filesCollection(function(er, s_col) {
      er ? openCallback(er): s_col.findOne({
...
Commands.prototype.write = function(data, options, callback) {
  ...
  function writePrepare(rsl, rej) {
    ...
    gc.filesCollection(function(er, filesCollection) {
      ...
      f_col.findOne({
        ...
        // !obj || obj._write != FileIs.ON_UPDATE
  ...
  function runWrite() {
    ...
    gc.writeBuffer(Buffer.isBuffer(data) ? data: new Buffer(data), EOF,
      finalClose, writeCallback);
  }
...
Commands.prototype.writeBuffer = function(buffer, EOF, finalClose, callback) {
  ...
  gc.writeStream = gc.writeStream.then(function() {
    return new Promise(writeProcessor).then(writeBufferCallback,
      writeBufferCallback);
  });
  return;
...
Commands.prototype.buildMongoObject = function(writing, callback) {
  ...
  var mongoObject = {
    ...
    'contentType': gs.contentType || gc.contentType,
...
Commands.prototype.close = function(callback) {
  ...
  var closingCallback = function(er) {
    if(er)
      return gc.errHandler(er, callback);
  ...
  gc.filesCollection(function(er, f_col) {
    ...
    f_col.update({
      ...
      er ? closingCallback(er): deleteOld();
    });
    ...
    function deleteOld() {
      gc.deleteFiles({
        ...
        filename: gc.filename,
        uploadDate: {
          $lte: new Date() - Commands.OLD_DATA_READABLE_MILLISECOND
        },

        _write: FileIs.STABLE,
        _no: {
          $ne: gc._no
        }
...
Commands.prototype.chunkIn = function(options, callback) {
  ...
  function afterStat(er, stats) {
    if(er)
      return gc.errHandler('stats failed ' + filepath, er, ee);
    if(!stats || stats.size == 0)
      return ee.emit('end');
    options.contents ? writeContent(): readStreamAndWrite();
  } // << -------- afterStat() --------

  function writeContent() {
    writeStreamChunk(typeof options.contents == 'string' ? new Buffer(
      options.contents): options.contents, true);
  } // << -------- writeContent() --------
 ...
 * @constant
 */
Commands.OLD_DATA_READABLE_MILLISECOND = 3000;
