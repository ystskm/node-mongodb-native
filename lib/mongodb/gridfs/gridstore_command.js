...
var OutDebug = process.env.NODE_DEBUG || true ? function(fn, message) {
...
Commands.prototype.eventLock = function(key, lockId, statOverwrite, callback) {
  ...
  function locked() {
    ...
    OutDebug('eventLock', function() {
      console.log('EVENT LOCK SUCCEEDED.', gc.filename, gc.gs.gsid);
    });
  ...
  function release(r_callback) {
    gc.upsetStatsData({
      ...
      OutDebug('eventLock', function() {
        console.log('EVENT LOCK RELEASE SUCCEEDED.', gc.filename, gc.gs.gsid);
      });
