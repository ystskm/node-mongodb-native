...
Commands.prototype.deleteFiles = function(key, chunks_del, callback) {
  ...
  function findcb(err, cur) {
    ...
    cur.each(function(err, doc) {
      ...
      if(!doc) {
        cur.rewind(), cur = null;
        return Promise.all(prms).then(delcb)['catch'](delcb);
      }
      ...
      prms.push(new Promise(function(rsl, rej) {
        col.remove({
          ...
          if(err) {
            console.log('REMOVE CALLBACK E?', err)
            return rej(err);
          }

          gc.deleteChunks(_id, function(err) {
            console.log('DELETE CHUNKS E?', err)
            err ? rej(err): rsl();
          });
