  ...
  function wpCountcb(err, cnt) {
    ...
    gc.getStatsData(function(err, stats) {
      ...
      function setWaitingTimeout() {
        ...
        var timeout = Commainds.DEFAULT_SAVE_RETRY_BASE_MILLISECOND
...
Commands.prototype.eventLock = function(key, lockId, statOverwrite, callback) {
  ...
  function upsetStat(stat) {
    ...
    Object.keys(val[key]).forEach(function(lk) {
      if(val[key][lk] + Commands.STAT_FORCE_UPDATE_LIMIT_MILLISECOND > _l.time)
        return;
      delete val[key][lk];
    });
...
//debug judgement
var Debug = process.env.DEBUG, DebugMatch = null, DebugLevel = 0;
