...
Commands.prototype.recoverCondition = function(now_stat, try_sel, callback) {
  ...
  if(ns_write_id
    ...
    return self.recover(false, callback), sel;
  }

  if(ns_write_keys.indexOf(Const.RecoverLock) != -1
    && Date.now() - ns_write[Const.RecoverLock] < Commands.STAT_FORCE_UPDATE_LIMIT_MILLISECOND)
...
Commands.prototype.recover = function(ns_write_id, callback) {
  ...
  function execRandomTimeRetryAfterRecover(err) {
    ...
    var stance = ns_write_id !== false ? 'Recovered file': 'Wait recoverly ';
    OutMessage.log('recover', stance + ' datas of: ' + self.gs.gsid + "; "
...
Commands.prototype.eventLock = function(key, lockId, statOverwrite, callback) {
  ...
  function setStatus(err, stat) {
    ...
    if(try_sel != null) {
      ...
      if(stat._writing == try_sel._writing)
        return upsetcb();
    }
  ...
  function upsetcb(err, n) {
    ...
    function progressCheckAndRetry() {
      ...
      try_sel = self.recoverCondition(now_stat, try_sel, getStatus);
    }
...
Commands.prototype.eventUnlock = function(callback) {
  ...
  var val = null, try_sel = null, now_stat = null;
  var no = null, is_mine = {};
