...
function errGen(mes, method) {
  ...
  return 'Error: ' + (mes ? mes + ' ': '') + 'at '
    + (method ? method + '()': '') + ' in gridCommands.';
...
Commands.prototype.open = function(callback) {
  ...
  function assignAndJudge(obj) {
    ...
    if(OpenModeIs.read(self.mode) && !obj)
      return self.errHandler(Err.ENOENT, callback);
...
var _reader = function(gc, options) {
  ...
  this.size = this.options.size || gc.length;
  this.offset = this.options.offset || 0;
  this.encode = this.gc.gs.contentType.slice(0, 4) == 'text'
    ? this.options.encode || 'utf8': null;
...
_reader.prototype.end = function(fn) {

  var self = this;

  // fn is callback for buffered reading
  if(typeof fn == 'function')
    this.on(DataEvent.Error, onError).on(DataEvent.End, onEnd);
  this.chunks = new Array(), this.exec();
  ...
  function onEnd(byteRead) {
    var chunks = Buffer.concat(self.chunks);
    fn.call(self.gc, null, byteRead, self.encode ? chunks.toString(self.encode)
      : chunks);
  }

};
_reader.prototype.exec = function() {

  var self = this, chunks = this.chunks;
  this.gc.countChunks(countcb);
  ...
  function _nth(n) {
    return function(err, callback) {
      ...
      function nthcb(err, chunk_origin) {
        ...
        self._read += chunk.length;

        if(self.offset <= self._read && self._read <= self.size) {
          chunks.push(chunk);
  ...
  function _fin(err) {
    ...
    if(err)
      return self._emitter.emit(DataEvent.Error, err);

    var byteRead = self._read;
    self._read = 0, self._emitter.emit(DataEvent.End, byteRead);
...
Commands.prototype.readChunks = function(callback) {
  callback(null, new _reader(this), this._files.forRead);
};
...
Commands.prototype.write = function(data, options, callback) {
  ...
  var self = this, EOF = false, finalClose = false;
...
Commands.prototype.writeBuffer = function(buffer, EOF, finalClose, callback) {
  ...
  if(typeof EOF == 'function')
    callback = EOF, finalClose = false, EOF = false;

  else if(typeof finalClose == 'function')
    callback = finalClose, finalClose = false, EOF = false;
  ...
  function writeBuffer(leftOverData) {
    ...
    var leftOverDataSize = self.currentChunk.chunkSize
      - self.currentChunk.cursorPosition;
    ...
    function wcb(err, chunk) {
      ...
      self.length += self.currentChunk.length();
...
Commands.prototype.eventUnlock = function(callback) {
  ...
  function setStatus(err, stat) {
    ...
    if(try_sel != null && stat._no == try_sel._no
      && stat.update.getTime() == try_sel.update.getTime()
      && stat._writing == try_sel._writing)
...
Commands.prototype.chunkIn = function(options, callback) {
  ...
  filepath = options.filePaht || this.gs.internalFilePath;
  ...
  return fs.stat(filepath, afterStat), ee;
