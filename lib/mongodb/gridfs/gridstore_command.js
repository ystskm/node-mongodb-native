...
Commands.prototype.open = function(callback) {
  ...
  function wpCountcb(err, cnt) {
    ...
    gc.getStatsData(function(err, stats) {
      ...
      OutMessage('waitProcess', 'recover: ' + gsid + ' (after ' + timeout
        + ' ms)');
      ...
      function setWaitingTimeout() {
        OutMessage('waitProcess', 'setTimeout condition: ' + gsid
          + '; WritingID: ' + el_id + ' for ' + gc.filename);

        wc.cnt = cnt, wc.obj = wc.objNow;
        timer.setTimeout(waitProcess, Commands.DEFAULT_SAVE_RETRY_MILLISECOND);
      }
...
Commands.prototype.recover = function(ns_write_id, callback) {
  ...
  // real retry log
  // TODO debug
  OutMessage('recover', 'Recover file datas of: ' + gsid + "; " + gc.filename
    + ' (ns_write_id = ' + ns_write_id + ')');
  ...
  function execRandomTimeRetryAfterRecover(err) {
    ...
    return timer.setTimeout(callback, time);
  }
    ...
    cur.toArray(function(err, arr) {
      ...
      function forEachData(data) {
        ...
        col.remove({
          ...
          chunks_del === true ? gc.deleteChunks(_id, checkLast): checkLast();
        });
