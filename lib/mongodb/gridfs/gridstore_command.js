...
// (2020.09.01 sakamoto) new Buffer to Buffer.alloc / Buffer.from
if(typeof Buffer.from != 'function' || typeof Buffer.alloc != 'function') {
  Buffer.from = x=>Buffer(x), Buffer.alloc = x=>Buffer(x);
}
...
Commands.prototype.errHandler = function(msg, err, handle) {
  ...
  if(gc.errorHandled || gc === PREVIOUS_HANDLED) {
...
_reader.prototype.end = function(endCallback) {
  ...
  var maxChunk = NULL, read = 0, chunks = Buffer.alloc(0);
...
Commands.prototype.write = function(data, options, callback) {
  ...
  function runWrite() {
    ...
    gc
      .writeBuffer(Buffer.isBuffer(data) ? data: Buffer.from(data), EOF, finalClose, writeCallback);
  }
...
Commands.prototype.close = function(callback) {
  ...
  var closingCallback = function(er) {
    ...
    try {
      ...
      // gc.errHandler(e, callback); => occurs recursion
      callback(e);
    }
  };
...
Commands.prototype.deleteFiles = function(key, chunks_del, callback) {
  ...
  if(gc.ObjectID == NULL) {
    return callback(); // The "gc" already closed by closing error.
  }
...
Commands.prototype.chunkIn = function(options, callback) {
  ...
  function readStreamAndWrite() {
    ...
    stream.on('error', function(e) {
      ...
      writeStreamChunk(Buffer.alloc(0), true);
      careLeak();
