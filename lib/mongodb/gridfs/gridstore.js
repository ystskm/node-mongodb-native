...
var Chunk = require('./chunk').Chunk, DbCommand = require('../commands/db_command').DbCommand;

var mime = require('mime');

//Buffer uses global.Buffer
...
var GsEvent = {
  STAT_BEGIN: 'stat_begin',
  STAT_END: 'stat_end',
  WRITE_WAIT: 'write_wait_begin',
 ...
 * @param fileTarget
 *          {string} The name for the file.
 * @param mode
 *          {?string} Set the mode for this file. Available modes:
 *          <ul>
 ...
 * 
 * @param options
 *          {?object} Optional properties to specify. Recognized keys:
 * 
 * <pre><code>
 *     {
 *       'root' : , // {string} root collection to use. Defaults to GridStore#DEFAULT_ROOT_COLLECTION
 *       'chunkType' : , // {string} mime type of the file. Defaults to GridStore#DEFAULT_CONTENT_TYPE
 *       'chunkSize' : , // {number} size for the chunk. Defaults to Chunk#DEFAULT_CHUNK_SIZE.
 *       'saveName' : , // {string} name in gridstore if you determine clearly
 *       'filePath' : , // {string} path to real-file if is not same as fileTarget.
 *       'metadata' : , // {object} arbitrary data the user wants to store
 *     }
 * </code></pre>
 * 
 * @see <a
 *      href="http://www.mongodb.org/display/DOCS/GridFS+Specification">MongoDB
 *      GridFS Specification</a>
 */
...
var GridStore = exports.GridStore = function(db, fileTarget, mode, options) {
  ...
  if(fileTarget instanceof db.bson_serializer.ObjectID) {
    ...
    this.referenceBy = ReferenceBy.ID;
    this.filesId = fileTarget;
    this.filename = null;

  } else {

    this.referenceBy = ReferenceBy.FILENAME;
    this.filesId = null;
  ...
  this.closedb = this.db.serverConfig.isConnected() ? false
    : this.options.closedb || false;

  this.root = this.options.root || GridStore.DEFAULT_ROOT_COLLECTION;
  // "fs."

  this.safe = this.options.safe == null ? true: this.options.safe;
  ...
  this.contentType = this.options.contentType || mime.lookup(this.filename)
    || GridStore.DEFAULT_CONTENT_TYPE;

  this.defaultChunkSize = (this.options.chunkSize || GridStore.DEFAULT_CHUNK_SIZE)
    - GridStore.MAX_CHUNK_HEADER_SIZE;

  // saveName > virtualPath > actualPath for save "filename"
  if(this.options.saveName)
    this.filename = this.options.saveName;

  // filePath > actualPath > filename for read "internalFilePath"
  if(this.options.filePath)
    this.internalFilePath = this.options.filePath;

  if(this.options.virtualPath) {

    this.internalVirtualPath = path.resolve(this.options.virtualPath,
      fileTarget);

    if(this.filename == fileTarget && !this.options.saveName)
      this.filename = this.internalVirtualPath;

  }

  if(this.options.actualPath) {

    this.internalFilePath = path.resolve(this.options.actualPath, fileTarget);

    if(this.filename == fileTarget && !this.options.saveName)
      this.filename = this.internalFilePath;

  }

  if(!this.internalFilePath)
    this.internalFilePath = this.filename;

  // extensional stats
  this.metadata = this.options.metadata;

  var dbnm = db.databaseName, flnm = this.filename;

  if(EmitterMap[dbnm] == null)
    EmitterMap[dbnm] = {};

  ['forStat', 'forWait'].forEach(function(key) {
  ...
  this.ge = EmitterMap[dbnm][flnm];

  if(!this.safe && 'r' == this.mode)
    this.db.slaveOk = false;
  ...
  this.__defineGetter__("md5", function() {
    return self.internalMd5;
  });
...
function listeningEmitter(dbnm, flnm) {
  ...
  [{
    qname: 'forStat',
    se_name: GsEvent.STAT_BEGIN,
    ee_name: GsEvent.STAT_END
  }, {
    qname: 'forWait',
    se_name: GsEvent.WRITE_WAIT,
    ee_name: GsEvent.WRITE_UNLOCK
  }].forEach(function(info) {

    //stats queue
    Queues[info.qname][dbnm][flnm] = [];

    //when statstart, queue and if only me, kick function
    emitter.on(info.se_name, function(fn, kick) {

      var q = Queues[info.qname][dbnm][flnm];

      q.push(fn);

      //        if(/file-io\/index.js/.test(flnm))
      //util.log('III[' + info.qname + '] l(' + q.length + ') f(' + flnm + ')');

      // if only me, execute
      if(q.length == 1)
        kick ? process.nextTick(q[0]): q.unshift(null);
...
GridStore.prototype.open = function(callback) {
  ...
  this.db.open(function(err) {
    ...
    */
    ...
    indexCheck(self.db, endOfOpen);

  });

  function endOfOpen() {

    new GridStoreCommands(self).open(function(err) {
...
GridStore.prototype.close = function(afterClose) {
  ...
  if(this.closedb == true && this.db.serverConfig.isConnected())
    return this.db.close(afterClose);
...
GridStore.prototype.statBegin = function(fn) {
  var ee = this.ge;
  ee.emit.call(ee, GsEvent.STAT_BEGIN, fn, true);
};

GridStore.prototype.statEnd = function() {
  var ee = this.ge;
  ee.emit.call(ee, GsEvent.STAT_END);
};

GridStore.prototype.waitForWrite = function(fn) {
  var ee = this.ge;
  ee.emit.call(ee, GsEvent.WRITE_WAIT, fn);
};

GridStore.prototype.informWriteUnlock = function() {
  var ee = this.ge;
  ee.emit.call(ee, GsEvent.WRITE_UNLOCK);
};
...
GridStore.ls = function(db, dirpath, options, callback) {
  ...
  var depth = options.depth == '*' ? '*': parseInt(options.depth) || 0;
  var emitter = new Emitter();
