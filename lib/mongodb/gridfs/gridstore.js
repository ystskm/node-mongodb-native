...
GridStore.ls = function(db, dirpath, options, callback) {
  ...
  var rega = dirpath.split('/');

  // '/' is set for dirpath care
  var regl = (rega.slice(-1).toString()).trim();
  regl.length === 0 && rega.pop();

  // if "options.filename" or "optons.depth" is set
  // it's the sign of directory search
  var dir_search_mode = false;
  if(options.filename || options.depth != null)
    dir_search_mode = true;

  var filename = null, depth = null;
  if(dir_search_mode) {
    filename = String(options.filename).replace(/\\/, "\\\\");
    depth = options.depth == '*' ? '*': parseInt(options.depth) || 0;
  }
  ...
  var regs = '^' + rega.join('\\/');
  // TODO javascript bridge.replace(/[^\.]\*/g, '.*');
  // => it's client matter?

  if(dir_search_mode) {
    // change directory regexp for each "depth" condition
    if(isNaN(depth))
      regs += '(\\/[^\\/]+)*';
    else if(depth === 1)
      regs += '(\\/[^\\/]+)?';
