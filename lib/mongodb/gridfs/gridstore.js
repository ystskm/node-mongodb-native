...
function indexCheck(db, callback) {
  ...
  var progress_ns = {};
  var progress_ee = new Emitter();
  progress_ee.on('error', function(e) {
    if(TRUE) {
  ...
  for( var ns in Indexes)
    (function(ns, indexes) {
      ...
      if(progress_ns[ns] == NULL) {
        progress_ns[ns] = 0;
      }

      progress_ns[ns]++;
      checkIndexes(ns, indexes);
  ...
  function checkIndexes(ns, indexes) {
    ...
    function getCurrent() {
      db.createCollection(ns, function(er, _col) {

        if(er) {
          progress_ee.emit('error', er);
          return;
        }
        col = _col;
        findSysIndexCol();
    ...
    function findSysIndexCol() {
      ...
      db.createCollection('system.indexes', function(er, sys_col) {
    ...
    function checkDiff(er, cur) {
      ...
      var dropKeyPatterns = [];
      cur.each(function(cur_er, doc) {

        if(cur_er) {
          progress_ee.emit('error', cur_er);
          return;
        }
        if(!doc) {
          cur.rewind(), cur = NULL;
...
GridStore.ls = function(db, dirpath, options, callback) {
  ...
  var LIMITED = is('number', limit), dup = 0;
  ...
  function opencb(er, cl) {
    if(er)
      return last(er);
    client = cl;
    db.collection(args.root + '.files', collcb);
  }
