...
// main
var GridStore = function(db, fileTarget, mode, options) {
  if(!(this instanceof GridStore))
    return new GridStore(db, ifileTarget, mode, options);
...
GridStore.ls = function(db, dirpath, options, callback) {
  ...
  var depth = regl != '' ? null: options.depth == '*' ? '*'
    : parseInt(options.depth) || 0;
  ...
  // depth: 'null' => file name search, '*' => any level, '(Number)' => limited
  depth !== null && regl == '' && rega.pop();

  var reg = '^' + rega.join('\\/'); // TODO javascript bridge.replace(/[^\.]\*/g, '.*');
  depth !== null && (typeof depth != 'number' ? function() {
    reg += '.+';
  }: function() {
    while(depth-- >= 0)
      reg += '\\/[^\\/]+';
