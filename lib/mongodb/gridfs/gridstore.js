 ...
 */
var fs = require('fs'), path = require('path'), util = require('util'), events = require('events'), Stream = require('stream');
var Chunk = require('./chunk').Chunk, DbCommand = require('../commands/db_command').DbCommand, ObjectID = require('bson').ObjectID;
var mime = require('mime'), Emitter = events.EventEmitter;
...
util.inherits(NonLeakEmitter, Emitter);
...
function indexCheck(db, callback) {
  ...
  // CAN'T USE "NonLeakEmitter" ,for assume that multiple "end" event.
...
GridStore.ls = function(db, dirpath, options, callback) {
  ...
  var emitter = new NonLeakEmitter();
