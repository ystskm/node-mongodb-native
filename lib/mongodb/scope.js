var Cursor2 = require('./cursor').Cursor, Readable = require('stream').Readable, utils = require('./utils'), inherits = require('util').inherits;
...
var Cursor = function Cursor(_scope_options, _cursor) {
  ...
  this.rewind = function() {
    var r = _cursor.rewind();
    Object.keys(_scope_options).forEach(function(k){
      delete _scope_options[k];
    });
    Object.keys(_cursor).forEach(function(k){
      delete _cursor[k];
    });
    _scope_options = _cursor =  null;
    return r;
  }
...
var Scope = function(collection, _selector, _fields, _scope_options) {
  ...
  // Set up the cursor
  var _cursor = new Cursor2(collection.db, collection, _selector, _fields,
    _scope_options);
  ...
  var writeOptions = {
    ...
    update: function(operations, callback) {
      ...
      if(_scope_options.upsert)
        update_options.upsert = _scope_options.upsert;
